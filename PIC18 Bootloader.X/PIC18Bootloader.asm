...
; ASM source line config statements for PIC18F46K22
...
; *****************************************************************************
    ORG     0
BootloaderStart:
    DigitalInput                ; set RX pin or RA4 pin as digital input on certain parts
...
;our bootloader checks pin RA4 and enter boot if pin is low
    ;btfsc   PORTA, RA4 ; if RA4 = 0, jump to application
	btfss	PORTA, RA4 ; if RA4 = 1, jump to application
    bra     BootloadMode
CheckAppVector:
...
; we need to make the RC7 pin digital in order to have serial working
	banksel	ANSELC
	bcf		ANSELC, ANSC7
;set RC2 pin to output and light the LED to signal the boot mode
#ifdef LED_ON_RC2
    bsf     LATC,   LATC2
    bcf     TRISC,  TRISC2
#endif
